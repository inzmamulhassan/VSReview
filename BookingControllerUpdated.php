...
class BookingController extends Controller
{
    ...
    {
        // Ihave no idea what pupose first if block is serving so I let it be as it is and will touch the second one
        ...
        if($this->repository->isAdmin($request->__authenticatedUser) || $this->repository->isSuperAdmin($request->__authenticatedUser))
        {
    ...
    {
        $adminSenderEmail =  $this->repository->getAdminEmailCredentials();
        $data = $request->all();
    ...
    public function distanceFeed(Request $request)
    {
        ...
        if ($admincomment || $session || $flagged || $manually_handled || $by_admin) {
            $affectedRows1 = $this->repository->updateJobs($jobid, $admincomment, $flagged, $session, $manually_handled, $by_admin);
        }
